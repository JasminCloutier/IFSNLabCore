function RLmodel_populationreciprocity(dataset)
    
    global invest reward k
    
    df = readtable(dataset);
    data = table2array(df);
    population_reciprocity = unique(data(:,7));
    
    k = 11;
    resultMatr = [];
    paramMat = [];
    for reciprocity=1:length(population_reciprocity) 
        popdata = data(data(:,5) == population_reciprocity(reciprocity),:);  % extract data of the subj
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for alpha = 0:0.3:1    % initial value of estimation for learning rate
            for beta = 0:2:6  % initial value of estimation for inverse temperature
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                resultMatr = [resultMatr;[population_reciprocity(reciprocity), alpha_minimized, beta_minimized, fval]];   % put these parameters in the resultMatr matrix
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if isempty(find(min(abs(fvals))))
            ...
            paramMat = [paramMat;[population_reciprocity(reciprocity), LR, IT, LL]];
        end
    end
    
    savename='RL_AgencyTrust_resultMatr_reciprocity';
    save([savename,'.mat'],'resultMatr');
    savename='RL_AgencyTrust_resultMat_reciprocity';
    save([savename,'.mat'],'resultMat');
    savename='RL_AgencyTrust_paramMat_reciprocity';
