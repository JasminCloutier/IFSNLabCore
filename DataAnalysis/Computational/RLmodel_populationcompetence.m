function RLmodel_populationcompetence(dataset)
    
    global invest reward k
    
    df = readtable(dataset);
    data = table2array(df);
    population_status = unique(data(:,6));
    
    k = 11;
    resultMatr = [];
    paramMat = [];
    for status=1:length(population_status) 
        popdata = data(data(:,6) == population_status(status),:);  % extract data of the subj
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for alpha = 0:0.3:1    % initial value of estimation for learning rate
            for beta = 0:2:6  % initial value of estimation for inverse temperature
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                resultMatr = [resultMatr;[population_status(status), alpha_minimized, beta_minimized, fval]];   % put these parameters in the resultMatr matrix
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if isempty(find(min(abs(fvals))))
            ...
            paramMat = [paramMat;[population_status(status), LR, IT, LL]];
        end
        
    end
    
    savename='RL_AgencyTrust_resultMatr_competence';
    save([savename,'.mat'],'resultMatr');
    savename='RL_AgencyTrust_resultMat_competence';
    save([savename,'.mat'],'resultMat');
    savename='RL_AgencyTrust_paramMat_competence';
