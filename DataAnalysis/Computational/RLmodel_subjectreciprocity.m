function RLmodel_subjectreciprocity(dataset)
    ...
    for subj=1:5%length(subjects) 
        ...
        sub_reciprocity = unique(subdata(:,7));
        for reciprocity=1:length(sub_reciprocity)
            subreciprocitydata = subdata(subdata(:,7) == sub_reciprocity(reciprocity),:); % extract data of subject by agency
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            invest = subreciprocitydata(:,8); % Money trusted is column 8
            reward = subreciprocitydata(:,9); % Partner return is column 9
            for alpha = 0:0.3:1    % initial value of estimation for learning rate
                for zeta = 0:2:6  % initial value of estimation for inverse temperature
                    ...
                    zeta_minimized = x(2);
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    resultMatr = [resultMatr;[subjects(subj), sub_reciprocity(reciprocity), alpha_minimized, zeta_minimized, fval]];   % put these parameters in the resultMatr matrix
                end
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            paramMat = [paramMat;[subjects(subj), sub_reciprocity(reciprocity), LR, IT, LL]];
        end
    end
    savename='RL_AgencyTrust_resultMatr_subreciprocity';
    save([savename,'.mat'],'resultMatr');
    savename='RL_AgencyTrust_resultMat_subreciprocity';
    save([savename,'.mat'],'resultMat');
    savename='RL_AgencyTrust_paramMat_subreciprocity';
